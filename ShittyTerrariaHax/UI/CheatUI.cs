...
{
	internal class CheatUI : UIState
	{
		...
		public override void OnInitialize()
		{
			...
			UIImageButton closeButton = new UIImageButton(ShittyTerrariaHax.Instance.GetTexture("UI/closeButton"));
			closeButton.OnClick += (e, el) => ShittyTerrariaHax.Instance.SetUIVisible(false);
			closeButton.Left.Set(250f, 0f);
			closeButton.Top.Set(1f, 0f);
			basePanel.Append(closeButton);

			UIText header = new UIText("Hax");
			header.HAlign = 0.5f;
			...
			addCheckbox(ref top, "Toggle", "Toggle ...Stuff...", nameof(MPlayer.toggle));
			addCheckbox(ref top, "Unhittable", "Enemies just walk right through you", nameof(MPlayer.unhittable));
			// fullbright
			// noclip
			// speed
			// open item spawner
			// open xray/ ore/tile finder
